...
const Controller = () => {
  ...
  const {run, loading, data} = useFetch(() =>
    getSetting({id: route.params.id, language: i18n.language}),
  );
  const {t, i18n} = useTranslation();
  ...
  function updateData(item, callback) {
    const findIndex = paramsList.current.findIndex(
      i => i.cmd === item.cmd__cmd,
    );
    ...
    //在这里发起保存，执行回调
    submitAdmin({id: route.params.id, data: paramsList.current})
      .then(res => {
        if (res.code == 200) {
          if (res.errs) {
    ...
    <ScrollView style={{flex: 1}}>
      <Loading loading={loading}>
        <AutoView
          ...
          <Start>
            <AutoView
              style={{width: 50, height: 30, backgroundColor: '#a5ce77'}}
            />
            <AutoText
              style={{
