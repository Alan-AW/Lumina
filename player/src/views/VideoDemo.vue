  ...
  <el-container class="player-container" :style="{ width: '100vw', height: '100vh' }">
    <el-main class="main-container">
      <el-col key="col" :style="{ height: '100%' }" :span="24" class="player-box active-player">
        <video-player :ref="`player`" :id="'player'" :rtspURL="rtspURL" :audioURL="audioURL" :downloadURL="downloadURL"
          :isLive="isLive" @downloadProgressUpdate="downloadProgressUpdate" @playProgressUpdate="playProgressUpdate"
          @downloadComplete="downloadComplete" @downloadError="downloadError" @playError="playError" />
      </el-col>
    </el-main>
    <el-footer height="40px">
      <el-row class="w-100">
        <el-col :span="22" class="h40 flex">
          <div :class="isPause || !isPlay ? 'play-icon' : 'pause-icon'" :title="isPause || !isPlay ? '播放' : '暂停'"
          ...
          <div class="sound-icon" :class="{ 'active': isMuting }" :title="isMuting ? '静音' : '声音'" @click="toggleMute">
          </div>
          ...
          <div class="config-set">
            ...
            <span v-show="!isLive">
              ...
              <el-date-picker size="mini" v-model="recordTimeRange" type="datetimerange" range-separator="至"
                start-placeholder="录像开始日期" end-placeholder="录像结束日期" :clearable="false" style="width: 315px;">
              </el-date-picker>
            </span>
